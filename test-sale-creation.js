// Script para probar la creación de una venta con productos
const admin = require('firebase-admin');

const serviceAccount = {
  "type": "service_account",
  "project_id": "zarzify",
  "private_key_id": "65bd6af1341571f53b281f9461cc75f8814c3854",
  "private_key": "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
  "client_email": "firebase-adminsdk-fbsvc@zarzify.iam.gserviceaccount.com",
  "client_id": "117905629515098321451",
  "auth_uri": "https://accounts.google.com/o/oauth2/auth",
  "token_uri": "https://oauth2.googleapis.com/token",
  "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
  "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/firebase-adminsdk-fbsvc%40zarzify.iam.gserviceaccount.com",
  "universe_domain": "googleapis.com"
};

admin.initializeApp({ credential: admin.credential.cert(serviceAccount) });
const db = admin.firestore();

async function testSaleCreation() {
  console.log('🧪 Probando creación de venta con productos...');
  
  try {
    const testSaleData = {
      business_id: '64085497-fa2a-438d-8926-150a21b2bac1', // Ferrezar
      cliente_id: '647dadfc-63cd-4f1d-83bd-0f5e9a36a236',
      metodo_pago: 'efectivo',
      descuento: 0,
      total: 15000,
      productos: [
        {
          id: 'test-product-1',
          nombre: 'Producto de Prueba 1',
          precio: 5000,
          cantidad: 2,
          subtotal: 10000
        },
        {
          id: 'test-product-2',
          nombre: 'Producto de Prueba 2',
          precio: 5000,
          cantidad: 1,
          subtotal: 5000
        }
      ],
      fecha_venta: new Date(),
      estado: 'completada',
      created_at: admin.firestore.FieldValue.serverTimestamp()
    };
    
    console.log('📤 Datos de prueba:', JSON.stringify(testSaleData, null, 2));
    
    // Crear la venta de prueba
    const docRef = await db.collection('sales').add(testSaleData);
    console.log(`✅ Venta de prueba creada con ID: ${docRef.id}`);
    
    // Verificar que se guardó correctamente
    const savedSale = await docRef.get();
    const savedData = savedSale.data();
    
    console.log('\n📊 Venta guardada:');
    console.log(`   ID: ${docRef.id}`);
    console.log(`   Business ID: ${savedData.business_id}`);
    console.log(`   Total: ${savedData.total}`);
    console.log(`   Productos: ${savedData.productos ? savedData.productos.length : 0}`);
    
    if (savedData.productos && savedData.productos.length > 0) {
      console.log('   📦 Productos guardados:');
      savedData.productos.forEach((product, index) => {
        console.log(`     ${index + 1}. ${product.nombre} - Cantidad: ${product.cantidad} - Precio: ${product.precio}`);
      });
    }
    
    // Limpiar la venta de prueba
    await docRef.delete();
    console.log('\n🧹 Venta de prueba eliminada');
    
  } catch (error) {
    console.error('❌ Error:', error);
  }
}

testSaleCreation().catch(console.error);
