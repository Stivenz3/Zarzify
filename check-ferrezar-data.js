// Script para verificar datos de Ferrezar en Firestore
const admin = require('firebase-admin');

const serviceAccount = {
  "type": "service_account",
  "project_id": "zarzify",
  "private_key_id": "65bd6af1341571f53b281f9461cc75f8814c3854",
  "private_key": "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
  "client_email": "firebase-adminsdk-fbsvc@zarzify.iam.gserviceaccount.com",
  "client_id": "117905629515098321451",
  "auth_uri": "https://accounts.google.com/o/oauth2/auth",
  "token_uri": "https://oauth2.googleapis.com/token",
  "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
  "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/firebase-adminsdk-fbsvc%40zarzify.iam.gserviceaccount.com",
  "universe_domain": "googleapis.com"
};

admin.initializeApp({ credential: admin.credential.cert(serviceAccount) });
const db = admin.firestore();

async function checkData() {
  try {
    console.log('🔍 Verificando datos para Ferrezar...');
    
    // Buscar el negocio Ferrezar
    const businesses = await db.collection('businesses').get();
    let ferrezarId = null;
    
    businesses.forEach(doc => {
      const data = doc.data();
      if (data.nombre === 'Ferrezar') {
        ferrezarId = doc.id;
        console.log('✅ Negocio Ferrezar encontrado:', doc.id);
      }
    });
    
    if (!ferrezarId) {
      console.log('❌ Negocio Ferrezar no encontrado');
      return;
    }
    
    // Verificar datos relacionados
    const [products, sales, clients, expenses] = await Promise.all([
      db.collection('products').where('business_id', '==', ferrezarId).get(),
      db.collection('sales').where('business_id', '==', ferrezarId).get(),
      db.collection('clients').where('business_id', '==', ferrezarId).get(),
      db.collection('expenses').where('business_id', '==', ferrezarId).get()
    ]);
    
    console.log('📊 Datos para Ferrezar:');
    console.log('  - Productos:', products.size);
    console.log('  - Ventas:', sales.size);
    console.log('  - Clientes:', clients.size);
    console.log('  - Gastos:', expenses.size);
    
    // Mostrar algunas ventas
    if (sales.size > 0) {
      console.log('\n💰 Primeras 3 ventas:');
      sales.docs.slice(0, 3).forEach(doc => {
        const data = doc.data();
        console.log('  - ID:', doc.id, 'Total:', data.total, 'Fecha:', data.fecha);
      });
    }
    
    // Mostrar algunos gastos
    if (expenses.size > 0) {
      console.log('\n💸 Primeros 3 gastos:');
      expenses.docs.slice(0, 3).forEach(doc => {
        const data = doc.data();
        console.log('  - ID:', doc.id, 'Concepto:', data.concepto, 'Monto:', data.monto, 'Fecha:', data.fecha);
      });
    }
    
  } catch (error) {
    console.error('❌ Error:', error);
  }
  process.exit(0);
}

checkData();
