// Script para corregir datos de ventas
const admin = require('firebase-admin');

// Usar la clave privada del archivo
const serviceAccount = {
  "type": "service_account",
  "project_id": "zarzify",
  "private_key_id": "65bd6af1341571f53b281f9461cc75f8814c3854",
  "private_key": "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
  "client_email": "firebase-adminsdk-fbsvc@zarzify.iam.gserviceaccount.com",
  "client_id": "117905629515098321451",
  "auth_uri": "https://accounts.google.com/o/oauth2/auth",
  "token_uri": "https://oauth2.googleapis.com/token",
  "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
  "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/firebase-adminsdk-fbsvc%40zarzify.iam.gserviceaccount.com",
  "universe_domain": "googleapis.com"
};

admin.initializeApp({ credential: admin.credential.cert(serviceAccount) });
const db = admin.firestore();

async function fixSalesData() {
  console.log('🔧 Corrigiendo datos de ventas...');
  
  try {
    const salesSnapshot = await db.collection('sales').get();
    let fixedCount = 0;
    
    for (const doc of salesSnapshot.docs) {
      const sale = doc.data();
      const saleId = doc.id;
      
      console.log(`\n💰 Venta: ${saleId}`);
      console.log(`   Fecha actual: ${sale.fecha_venta || sale.fecha}`);
      console.log(`   Método de pago: ${sale.metodo_pago || 'No definido'}`);
      
      const updates = {};
      let needsUpdate = false;
      
      // Corregir fecha_venta si no existe o es incorrecta
      if (!sale.fecha_venta || sale.fecha_venta === '2000-01-01') {
        if (sale.fecha) {
          updates.fecha_venta = sale.fecha;
        } else if (sale.created_at) {
          updates.fecha_venta = sale.created_at;
        }
        needsUpdate = true;
        console.log(`   ✅ Fecha corregida`);
      }
      
      // Agregar método de pago si no existe
      if (!sale.metodo_pago) {
        updates.metodo_pago = 'efectivo';
        needsUpdate = true;
        console.log(`   ✅ Método de pago agregado: efectivo`);
      }
      
      if (needsUpdate) {
        await db.collection('sales').doc(saleId).update(updates);
        fixedCount++;
      } else {
        console.log(`   ✅ Datos correctos`);
      }
    }
    
    console.log(`\n🎉 Proceso completado. ${fixedCount} ventas corregidas.`);
    
  } catch (error) {
    console.error('❌ Error:', error);
  }
}

fixSalesData().catch(console.error);
