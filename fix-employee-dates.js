// Script para corregir fechas de empleados
const admin = require('firebase-admin');

// Usar la clave privada del archivo
const serviceAccount = {
  "type": "service_account",
  "project_id": "zarzify",
  "private_key_id": "65bd6af1341571f53b281f9461cc75f8814c3854",
  "private_key": "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
  "client_email": "firebase-adminsdk-fbsvc@zarzify.iam.gserviceaccount.com",
  "client_id": "117905629515098321451",
  "auth_uri": "https://accounts.google.com/o/oauth2/auth",
  "token_uri": "https://oauth2.googleapis.com/token",
  "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
  "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/firebase-adminsdk-fbsvc%40zarzify.iam.gserviceaccount.com",
  "universe_domain": "googleapis.com"
};

admin.initializeApp({ credential: admin.credential.cert(serviceAccount) });
const db = admin.firestore();

async function fixEmployeeDates() {
  console.log('🔧 Corrigiendo fechas de empleados...');
  
  try {
    const employeesSnapshot = await db.collection('employees').get();
    let fixedCount = 0;
    
    for (const doc of employeesSnapshot.docs) {
      const employee = doc.data();
      const employeeId = doc.id;
      
      console.log(`\n👤 Empleado: ${employee.nombre || 'Sin nombre'}`);
      console.log(`   Fecha actual: ${employee.fecha_contratacion}`);
      
      // Si la fecha es null, undefined, o una fecha inválida, establecer una fecha por defecto
      if (!employee.fecha_contratacion || 
          employee.fecha_contratacion === '1999-12-31' ||
          new Date(employee.fecha_contratacion).getFullYear() === 1999) {
        
        // Usar la fecha de creación como fecha de contratación por defecto
        const defaultDate = employee.created_at?.toDate ? 
          employee.created_at.toDate() : 
          new Date();
        
        await db.collection('employees').doc(employeeId).update({
          fecha_contratacion: admin.firestore.Timestamp.fromDate(defaultDate)
        });
        
        console.log(`   ✅ Corregido a: ${defaultDate.toLocaleDateString()}`);
        fixedCount++;
      } else {
        console.log(`   ✅ Fecha correcta: ${employee.fecha_contratacion}`);
      }
    }
    
    console.log(`\n🎉 Proceso completado. ${fixedCount} empleados corregidos.`);
    
  } catch (error) {
    console.error('❌ Error:', error);
  }
}

fixEmployeeDates().catch(console.error);
