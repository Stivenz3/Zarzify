// Script para corregir fechas de gastos
const admin = require('firebase-admin');

// Usar la clave privada del archivo
const serviceAccount = {
  "type": "service_account",
  "project_id": "zarzify",
  "private_key_id": "65bd6af1341571f53b281f9461cc75f8814c3854",
  "private_key": "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
  "client_email": "firebase-adminsdk-fbsvc@zarzify.iam.gserviceaccount.com",
  "client_id": "117905629515098321451",
  "auth_uri": "https://accounts.google.com/o/oauth2/auth",
  "token_uri": "https://oauth2.googleapis.com/token",
  "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
  "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/firebase-adminsdk-fbsvc%40zarzify.iam.gserviceaccount.com",
  "universe_domain": "googleapis.com"
};

admin.initializeApp({ credential: admin.credential.cert(serviceAccount) });
const db = admin.firestore();

async function fixExpensesDates() {
  console.log('🔧 Corrigiendo fechas de gastos...');
  
  try {
    const expensesSnapshot = await db.collection('expenses').get();
    let fixedCount = 0;
    
    for (const doc of expensesSnapshot.docs) {
      const expense = doc.data();
      const expenseId = doc.id;
      
      console.log(`\n💸 Gasto: ${expense.concepto || 'Sin concepto'}`);
      console.log(`   Fecha actual: ${expense.fecha_pago || expense.fecha}`);
      
      const updates = {};
      let needsUpdate = false;
      
      // Corregir fecha_pago si no existe o es incorrecta
      if (!expense.fecha_pago || expense.fecha_pago === '2000-01-01') {
        if (expense.fecha) {
          updates.fecha_pago = expense.fecha;
        } else if (expense.created_at) {
          updates.fecha_pago = expense.created_at;
        } else {
          updates.fecha_pago = new Date();
        }
        needsUpdate = true;
        console.log(`   ✅ Fecha corregida`);
      }
      
      if (needsUpdate) {
        await db.collection('expenses').doc(expenseId).update(updates);
        fixedCount++;
      } else {
        console.log(`   ✅ Fecha correcta`);
      }
    }
    
    console.log(`\n🎉 Proceso completado. ${fixedCount} gastos corregidos.`);
    
  } catch (error) {
    console.error('❌ Error:', error);
  }
}

fixExpensesDates().catch(console.error);
